...
df <- read.csv(input_file) %>%
    ...
    summarize(balanceratio=sum(balance[strategy=="default"])/sum(balance[strategy=="balanced"]))

plotWithData <- function(data) {
    ggplot(data, aes(x=reorder(factor(type), balanceratio),
                     y=balanceratio,
                     fill=ifelse(balanceratio > 1, "Original", "Balanced"))) +

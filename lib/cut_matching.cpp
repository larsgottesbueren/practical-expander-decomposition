...
namespace CutMatching {
  void Solver::Initialize(Parameters params) {
    ...
    int count = 0;
    for (auto u : *subdivGraph) { if ((*subdivisionIdx)[u] >= 0) { (*subdivisionIdx)[u] = count++; } }
  ...
  Solver::Solver(UnitFlow::Graph* g, UnitFlow::Graph* subdivG,
    ...
    : graph(g), subdivGraph(subdivG), randomGen(randomGen),
      ...
      numSplitNodes(subdivGraph->size() - graph->size()) { assert(graph->size() != 0 && "Cut-matching expected non-empty subset."); }
  ...
  std::pair<std::vector<int>, std::vector<int>> Solver::KRVCutStep(
    ...
    for (auto u : *subdivGraph) {
      const int idx = (*subdivisionIdx)[u];
      if (idx >= 0) { perm.emplace_back(flow[idx], u); }
    }
    size_t mid = perm.size() / 2;
    std::nth_element(perm.begin(), perm.begin() + mid, perm.end());
    std::vector<int> axLeft, axRight;
    for (size_t i = 0; i < mid; ++i) { axLeft.push_back(perm[i].second); }
    for (size_t i = mid; i < perm.size(); ++i) { axRight.push_back(perm[i].second); }
  ...
  std::pair<std::vector<int>, std::vector<int>> Solver::RSTCutStep(
    ...
    } else { std::reverse(axRight.begin(), axRight.end()); }
    ...
    if (params.balancedCutStrategy) { while (axRight.size() > axLeft.size()) { axRight.pop_back(); } } else {
      while ((int) axLeft.size() * 8 > curSubdivisionCount) { axLeft.pop_back(); }
    }

    while (axLeft.size() > axRight.size()) { axLeft.pop_back(); }
  ...
  Solver::proposeCut(const std::vector<double>& flow, const Parameters& params) const {
    if (params.krv_step_first && subdivGraph->removedSize() == 0) { return KRVCutStep(flow, params); } else { return RSTCutStep(flow, params); }
  }
  ...
  void Solver::RemoveCutSide(const std::vector<UnitFlow::Vertex>& cutLeft, const std::vector<UnitFlow::Vertex>& cutRight,
    ...
    if (subdivGraph->globalVolume(cutLeft.begin(), cutLeft.end()) < subdivGraph->globalVolume(cutRight.begin(), cutRight.end())) {
      for (auto u : cutLeft) { removed.insert(u); }
    } else { for (auto u : cutRight) { removed.insert(u); } }
    ...
    for (auto u : removed) {
      if ((*subdivisionIdx)[u] == -1) { graph->remove(u); }
  ...
  Result Solver::computeInternal(Parameters params) {
    ...
    for (int i = 0; i < params.num_flow_vectors; ++i) { flow_vectors.push_back(randomUnitVector()); }
    ...
    for (; iterations < iterationsToRun &&
      ...
      if (potential <= ProjectedPotentialConvergenceThreshold()) {
        if (result.iterationsUntilValidExpansion2 == std::numeric_limits<int>::max()) { result.iterationsUntilValidExpansion2 = iterations; }
        if (params.use_potential_based_dynamic_stopping_criterion) { break; }
      }
      ...
      if (axLeft.empty() || axRight.empty()) { break; }

      subdivGraph->reset();
      for (const auto u : axLeft) { subdivGraph->addSource(u, 1); }
      for (const auto u : axRight) { subdivGraph->addSink(u, 1); }
      ...
      if (hasExcess.empty()) { VLOG(3) << "\tAll flow routed."; } else {
  ...
  Result Solver::compute(Parameters params) {
    ...
    while (true) {
      ...
      if (result.iterationsUntilValidExpansion2 == std::numeric_limits<int>::max() && result.type != Result::Expander) { return result; }

...
struct Timer {
    ...
    Timepoint Start() {
        if (running)
            throw std::runtime_error("Called Start() but timer is already running");
    ...
    Duration Stop() {
        if (!running)
            throw std::runtime_error("Timer not running but called Stop()");
    ...
    Duration Restart() {
        if (!running)
            throw std::runtime_error("Timer not running but called Restart()");
        auto finish = std::chrono::high_resolution_clock::now();
        Duration duration = finish - start;
        total_duration += duration;
        start = finish;
...
enum Timing { ProposeCut, FlowMatch, Match, Misc, FlowTrim, ConnectedComponents, CutHeuristics, LAST_TIMING };
...
struct Timings {
    ...
    void Print() {
        ...
        for (int i = 0; i < LAST_TIMING; ++i) {
            std::cout << TimingNames[i] << "\t\t" << std::setprecision(3) << durations[i].count() << "\t\t" << 100.0 * durations[i].count() / total.count()
                      << "%" << std::endl;
        }
